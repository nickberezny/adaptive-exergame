...
my_filename = 'emg_data'  # with full path if target folder different from current folder (do not leave trailing /)
...
data_record = np.zeros((chans_in, 1))
    ...
    with nidaqmx.Task() as task_in:
        ...
        while running:  # make this adapt to number of channels automatically
            ...
            if(plot_emg):
                ...
                ax4.plot(data[3, -sampling_freq_in * 5:].T)
                ax5.plot(data[4, -sampling_freq_in * 5:].T)
                ax6.plot(data[5, -sampling_freq_in * 5:].T)
                ax7.plot(data[6, -sampling_freq_in * 5:].T)
                ax8.plot(data[7, -sampling_freq_in * 5:].T)
...
with open(filename + extension, 'w') as f:
    f.write(str(time_start.hour) + "," + str(time_start.minute) + ","  + str(time_start.second) + ","  + str(time_start.microsecond)[:-3] + ","  "\n")
    f.write(str(time_end.hour) + "," + str(time_end.minute) + ","  + str(time_end.second) + ","  + str(time_end.microsecond) + ","  "\n")
    f.write(str(sampling_freq_in) + "\n")
...
ax1.plot(data_record[0, 10:].T)  # note the exclusion of the first 10 iterations (automatically zoomed in plot)
ax2.plot(data_record[1, 10:].T)
ax3.plot(data_record[2, 10:].T)
ax4.plot(data_record[3, 10:].T)
ax5.plot(data_record[4, 10:].T)
ax6.plot(data_record[5, 10:].T)
ax7.plot(data_record[6, 10:].T)
ax8.plot(data_record[7, 10:].T)
# Label formatting ...
ax8.set_xlabel('time [s]')
#ax1.set_ylabel('voltage [V]')
#ax2.set_ylabel('voltage [V]')
